...
#
# The command shape below (`codex exec --sandbox <level> -m <model> "<prompt>"`) is
# built from Codex's own current non-interactive-mode documentation, but it has never
# been run against a real CLI from this repository — the evidentiary bar Task 16 sets
# (design.md: "flags verified by the source repository's own production use") is not
# met for this provider. A plausible invocation that silently does nothing is worse
# than an honest stub: this one refuses to `run` and says exactly why.
#
# To promote this adapter to ADAPTER_STATUS=verified: run it for real against a Codex
# CLI, confirm the headless flag, the model flag, the tool-permission (`--sandbox`)
# flag and the system-prompt handoff all behave as this file assumes, fix whatever is
# wrong, flip the line above, and re-run tools/init.sh so the steward and review
# workflows pick it up (see AGENTS.md-adjacent docs/runbooks and Task 21's adoption
# interview, which gates on exactly this line).
set -euo pipefail
set +x   # never trace: a `set -x` here would print AGENT_AUTH_TOKEN into a public log.

CLI_BIN="${CODEX_BIN:-codex}"
...
print_argv() {
  ...
  echo "(UNVERIFIED: the system-prompt handoff for \$AGENT_SYSTEM_PROMPT_FILE is not" \
    "yet confirmed against a real Codex CLI — see $ADAPTER_DOCS_URL)"
}
...
case "$verb" in
  ...
  run)
    ...
    ;;
  *)
    echo "codex.sh: unknown verb '$verb' (expected print-argv|run|status)" >&2
    exit 2
    ;;
esac
