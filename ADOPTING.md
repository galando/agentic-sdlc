...
| `{{PLACEHOLDER}}` | README.md, docs/QUALITY-GATES.md | a floor's symbolic representation (see the four floor tokens below); this occurrence documents the convention and is not itself a live token. | n/a — documents the {{...}} syntax itself, not a real token |
...
| `{{PROVIDER}}` | .agents/config.yml | claude-code \| codex \| gemini-cli \| compatible-endpoint. | tools/init.sh (interview) |
